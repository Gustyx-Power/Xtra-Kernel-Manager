...
class AppProfileService : Service() {
  ...
  override fun onCreate() {
    ...
    Log.d(
        TAG,
        "AppProfileService started successfully with default refresh rate: ${defaultRefreshRate}Hz",
    )
  }
          ...
          while (isActive) {
            try {
              ...
              if (foregroundPackage != null && foregroundPackage != lastForegroundPackage) {
                ...
                val profile = appProfiles.find { it.packageName == foregroundPackage && it.enabled }
  ...
  private fun applyProfile(profile: AppProfile) {
    serviceScope.launch {
      ...
      try {
        val refreshRateInfo =
            if (profile.refreshRate > 0) ", RefreshRate=${profile.refreshRate}Hz" else ""
        Log.d(
            TAG,
            "Applying profile for ${profile.appName}: Governor=${profile.governor}, Thermal=${profile.thermalPreset}$refreshRateInfo",
        )
        ...
        val toastMessage =
            if (profile.refreshRate > 0) {
              ...
              getString(
                  R.string.per_app_profile_applied,
                  profile.appName,
                  profile.governor,
                  profile.thermalPreset,
              )
            }
          ...
          if (clusters.isNotEmpty()) {
            clusters.forEach { cluster ->
              try {
                ...
                Log.e(
                    TAG,
                    "Failed to set governor for cluster ${cluster.clusterNumber}: ${clusterEx.message}",
                )
              }
            }
        ...
        Log.d(
            TAG,
            "Profile applied for ${profile.appName}: governor=$governorSuccess, thermal=$thermalSuccess, refreshRate=$refreshRateSuccess",
        )

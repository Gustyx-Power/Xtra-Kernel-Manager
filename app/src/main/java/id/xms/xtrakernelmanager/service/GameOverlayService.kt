    ...
    pollingJob =
        CoroutineScope(Dispatchers.Main).launch {
          while (true) {
            ...
            cpuLoad = withContext(Dispatchers.IO) { gameOverlayUseCase.getCPULoad() }
            ...
            gpuLoad = withContext(Dispatchers.IO) { gameOverlayUseCase.getGPULoad() }
  ...
  private fun setDND(enabled: Boolean) {
    ...
    CoroutineScope(Dispatchers.Main).launch {
      ...
      result
          ...
          .onFailure { error -> showToast("DND error: ${error.message}") }
    }
  }
  ...
  private fun clearRAM() {
    ...
    CoroutineScope(Dispatchers.Main).launch {
      ...
      result
          ...
          .onFailure { showToast("Gagal membersihkan RAM") }
    }
  }
    ...
    CoroutineScope(Dispatchers.Main).launch {
      ...
      withContext(Dispatchers.IO) {
        ...
        RootManager.executeCommand(
            "am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d file://$filename"
        )
      }

...
object NotificationHelper {
    ...
    fun generateIcon(
        ...
        if (type == "battery_icon") {
             return Icon.createWithResource(context, if (charging) R.drawable.ic_battery_charging else R.drawable.ic_battery)
        }
        ...
        val text = when (type) {
            ...
            "current" -> "${kotlin.math.abs(current)}" // Show absolute value? Usually users want magnitude
            "voltage" -> "${voltage}"
            "power" -> {

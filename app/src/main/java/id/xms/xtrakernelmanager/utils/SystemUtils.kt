...
object SystemUtils {
  ...
  suspend fun getKernelVersion(): String =
      withContext(Dispatchers.IO) { System.getProperty("os.version") ?: "Unknown" }

  suspend fun getDeviceModel(): String =
      withContext(Dispatchers.IO) { "${Build.MANUFACTURER} ${Build.MODEL}" }
  ...
  */
  ...
  suspend fun isMediaTekDevice(): Boolean =
      withContext(Dispatchers.IO) {
        val soc = RootManager.readFile("/sys/devices/soc0/soc_id").getOrNull()?.lowercase() ?: ""

...
class KernelRepository {
    ...
    private suspend fun getCoreInfo(coreNum: Int, cluster: Int): CoreInfo {
        ...
        val currentFreq = RootManager.executeCommand("cat $basePath/cpufreq/scaling_cur_freq 2>/dev/null")
            .getOrNull()?.trim()?.toIntOrNull() ?: 0
        val minFreq = RootManager.executeCommand("cat $basePath/cpufreq/cpuinfo_min_freq 2>/dev/null")
            .getOrNull()?.trim()?.toIntOrNull() ?: 0
        val maxFreq = RootManager.executeCommand("cat $basePath/cpufreq/cpuinfo_max_freq 2>/dev/null")
            .getOrNull()?.trim()?.toIntOrNull() ?: 0
        val governor = RootManager.executeCommand("cat $basePath/cpufreq/scaling_governor 2>/dev/null")
    ...
    suspend fun getGPUInfo(): GPUInfo = withContext(Dispatchers.IO) {
        val glesRaw = RootManager.executeCommand("dumpsys SurfaceFlinger | grep GLES")
            .getOrNull()?.trim() ?: ""
        ...
        if (glesRaw.contains("GLES:")) {
            ...
            renderer = parts.firstOrNull { Regex("adreno|mali|powervr|nvidia", RegexOption.IGNORE_CASE).containsMatchIn(it) }
                ?: (parts.getOrNull(1) ?: "Unknown")
            vendor = parts.firstOrNull { Regex("qualcomm|arm|nvidia|imagination", RegexOption.IGNORE_CASE).containsMatchIn(it) }
                ?: when {
                    ...
                    renderer.contains("NVIDIA", true) -> "NVIDIA"
                        ...
                        val soc = RootManager.executeCommand("getprop ro.hardware")

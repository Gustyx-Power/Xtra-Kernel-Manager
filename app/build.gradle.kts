...
android {
    ...
    buildTypes {
        ...
        splits {
            abi {
                isEnable = true
                reset()
                include("arm64-v8a", "armeabi-v7a") // Only ARM architectures
    ...
    dependencies {
        ...
        // Chart Library
        implementation("com.patrykandpatrick.vico:compose-m3:2.3.0-alpha.1")
    ...
    abstract class SendTelegramMessageTask : DefaultTask() {
        ...
        fun sendMessage() {
            ...
            val currentAppVersion =
                appVersionName.getOrElse(project.android.defaultConfig.versionName ?: "N/A")
            val currentAppPackage =
                appPackageName.getOrElse(project.android.defaultConfig.applicationId ?: "N/A")
            ...
            fun editTelegramMessage(messageId: Int, text: String) {
                val url =
                    "https://botapi.arasea.dpdns.org/bot${telegramBotToken.get()}/editMessageText"
                val jsonPayload = """
            ...
            fun pinTelegramMessage(messageId: Int) {
                val url =
                    "https://botapi.arasea.dpdns.org/bot${telegramBotToken.get()}/pinChatMessage"
                val jsonPayload = """
            ...
            val osName = try {
                ...
                prettyNameLine?.substringAfter("=")?.removeSurrounding("\"")
            ...
            val processor = try {
                ...
                val output = process.inputStream.bufferedReader().readText()
                process.waitFor()
                output.lines().find { it.startsWith("model name") }?.substringAfter(":")?.trim()
                    ?: osArch
            } catch (e: Exception) {
            ...
            val (totalRamGb, freeRamGb, usedRamGb) = try {
                ...
                val memTotalKb =
                    lines.find { it.startsWith("MemTotal:") }?.substringAfter(":")?.trim()
                        ?.removeSuffix(" kB")?.toLongOrNull() ?: 0L
                val memFreeKb =
                    lines.find { it.startsWith("MemFree:") }?.substringAfter(":")?.trim()
                        ?.removeSuffix(" kB")?.toLongOrNull() ?: 0L
                // MemAvailable is generally a better indicator of free memory
                val memAvailableKb =
                    lines.find { it.startsWith("MemAvailable:") }?.substringAfter(":")?.trim()
                        ?.removeSuffix(" kB")?.toLongOrNull() ?: memFreeKb
                ...
                Triple(
                    ...
                    String.format("%.2f GB", freeGb),
                    String.format("%.2f GB", usedGb)
                )
            ...
            if (buildStatus == "FAILED") {
                val failedTasks =
                    project.gradle.taskGraph.allTasks.filter { it.state.failure != null }
                if (failedTasks.isNotEmpty()) {
                    val errorDetails = failedTasks.joinToString(separator = "\n") { task ->
                        val taskName = task.path
                        val errorMessage =
                            task.state.failure?.message?.lines()?.firstOrNull()?.trim()
            ...
            if (buildMsgId != null) {
                editTelegramMessage(
                    buildMsgId,
                    if (buildStatus == "SUCCESS") "✅ Build finished successfully!" else "❌ Build failed!"
                )
            }
    ...
    abstract class UploadApkToTelegramTask : DefaultTask() {
        ...
        fun uploadApk() {
            ...
            logger.lifecycle(
                "Attempting to upload APK: ${currentApkFile.name} (Size: ${
                    "%.2f".format(
                        fileSizeMb
                    )
                } MB)"
            ...
            HttpClients.custom().setDefaultRequestConfig(requestConfig).build().use { httpClient ->
                ...
                entityBuilder.addTextBody(
                    "caption",
                    caption,
                    org.apache.http.entity.ContentType.TEXT_PLAIN.withCharset("UTF-8")
                )
...
// 3. Task untuk mengirim notifikasi status build
    val notifyBuildStatusToTelegram by tasks.registering(SendTelegramMessageTask::class) {
        ...
        appProjectName.convention(project.provider {
